	...
	$.ajax({
		...
		success: function(data){
			...
			while (tracknum < 5)
			{
				...
				var firstName = data.message.body.track_list[tracknum].track.track_name;
				console.log(firstName);
				var firstArtist = data.message.body.track_list[tracknum].track.artist_name;
				console.log(firstArtist);
				makeChart (firstName, firstArtist);
				tracknum++;
			}
...
function makeChart (name, artist){
	var chartTrack = "";
	chartTrack += "<div class='trackName'>";
	chartTrack += name + "'/>";
	chartTrack += "<div class='trackArtist'>" + artist + "</div>";
	chartTrack += "</div>";

	console.log ("in make CHART");
	
	$('.track').append(chartTrack);
}
